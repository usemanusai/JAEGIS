...
class WorkspaceAnalyzer {
    ...
    async analyzeProject(workspaceFolder) {
        ...
        const projectType = this.determineProjectType(packageJson, requirementsTxt, cargoToml, pomXml);
        const framework = this.detectFramework(packageJson);
        const language = this.detectLanguage(packageJson, requirementsTxt, cargoToml, pomXml);
        // Analyze project characteristics
     ...
     */
    async checkExistingJaegisSetup(workspaceFolder) {
     ...
     */
    async generateRecommendations(projectAnalysis) {
    ...
    generateActionRecommendations(analysis) {
        ...
        if (analysis.hasAuthentication && !analysis.dependencies.some(dep => dep.includes('security') || dep.includes('auth') || dep.includes('jwt'))) {
    ...
    async readRequirementsTxt(projectPath) {
        try {
    ...
    async readCargoToml(projectPath) {
        try {
    ...
    // Detection methods
    detectDatabase(packageJson, requirements) {
        if (packageJson) {
            const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
            const dbLibs = ['mongoose', 'sequelize', 'typeorm', 'prisma', 'pg', 'mysql', 'sqlite3'];
            return dbLibs.some(lib => deps[lib]);
        }
        return requirements.some(req => req.includes('django') || req.includes('sqlalchemy') || req.includes('psycopg2'));
    }
    detectAuthentication(packageJson, requirements) {
        if (packageJson) {
            const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
            const authLibs = ['passport', 'jsonwebtoken', 'bcrypt', 'auth0', 'firebase-auth'];
            return authLibs.some(lib => deps[lib]);
        }
        return requirements.some(req => req.includes('django-auth') || req.includes('flask-login') || req.includes('jwt'));
    }
    detectFrontend(packageJson) {
        if (!packageJson)
            return false;
        const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
    ...
    detectBackend(packageJson, requirements) {
        if (packageJson) {
            const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
            const backendLibs = ['express', 'fastify', '@nestjs/core', 'koa'];
            if (backendLibs.some(lib => deps[lib]))
                return true;
        }
        return requirements.some(req => req.includes('django') || req.includes('flask') || req.includes('fastapi'));
    }
    async detectDocker(projectPath) {
        try {
    ...
    async detectKubernetes(projectPath) {
        try {
    ...
    detectTests(packageJson, requirements) {
        if (packageJson) {
            const deps = { ...packageJson.dependencies, ...packageJson.devDependencies };
        ...
        return requirements.some(req => req.includes('pytest') || req.includes('unittest') || req.includes('nose'));
    }
    async detectCICD(projectPath) {
        const cicdPaths = [
            '.github/workflows',
            '.gitlab-ci.yml',
            'azure-pipelines.yml',
    ...
    calculateComplexity(packageJson, requirements, features) {
        let score = 0;
        ...
        if (features.hasAuthentication)
            score += 1;
        if (features.hasFrontend && features.hasBackend)
            score += 2;
    ...
    extractDependencies(packageJson, requirements) {
        const deps = [];
    ...
    recommendMode(projectType, complexity) {
    ...
    recommendAgents(projectType, features) {
        const agents = ['john', 'fred']; // Always include PM and Architect
        if (features.hasFrontend)
            agents.push('jane');
        if (features.hasAuthentication || features.hasDatabase)
            agents.push('sage');
        if (features.hasDocker || features.hasKubernetes)
            agents.push('alex');
        return agents;
    }
    calculateConfidence(projectType, framework, language) {
    ...
    determineProjectType(packageJson, requirements, cargoToml, pomXml) {
    ...
    detectLanguage(packageJson, requirements, cargoToml, pomXml) {
        if (packageJson) {
